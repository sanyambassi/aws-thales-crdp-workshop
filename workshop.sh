...
PUBLIC_IP=$(aws cloudformation describe-stacks --stack-name <your-stack-name> --query  "Stacks[0].Outputs[?OutputKey=='PublicIPAddress'].OutputValue" --output text)
...
microk8s kubectl create secret generic regtoken --from-literal=reg_token="$reg_token"
